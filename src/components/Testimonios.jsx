...
const TESTIMONIOS_DEMO = [
  { estrellas: 5, comentario: "Antes de conocerlos, organizar nuestro matrimonio era un estrés. Al llegar a Espacio Casona nos solucionaron todo con una atención increíble.", nombre: "Carlos R.", rol: "Novio", aprobado: true },
  { estrellas: 5, comentario: "Excelente lugar para eventos de empresa. Los salones son amplios y el entorno natural le dio un toque especial a nuestra jornada de capacitación.", nombre: "Elena V.", rol: "Recursos Humanos", aprobado: true },
  { estrellas: 5, comentario: "Celebramos los 50 años de mi padre aquí. La comida, el espacio y la atención fueron de primer nivel. Recomendado 100%.", nombre: "Martina S.", rol: "Invitada", aprobado: true }
];
...
const Testimonios = () => {
  ...
  const cargarTestimonios = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // 🔥 SOLO MOSTRAR RESEÑAS APROBADAS POR EL ADMIN
        const aprobadas = data.filter(t => t.aprobado === true || t.aprobado === undefined);
        setTestimonios(aprobadas.length > 0 ? aprobadas : TESTIMONIOS_DEMO);
      } else {
  ...
  // 🔥 Abrir Modal con Guardia de Seguridad
  ...
  // 🔥 Guardar Reseña - queda en "Pendiente" hasta que admin la apruebe
  const handleSave = async () => {
    ...
    const nuevoTestimonio = {
      ...
      aprobado: false, // 🔥 Empieza como pendiente hasta que el admin la apruebe
    ...
    try {
      ...
      if (response.ok) {
        Swal.fire({ 
          ...
          text: 'Tu comentario será revisado y publicado pronto.',
          timer: 2500, 
      ...
      Swal.fire({ 
        ...
        text: 'Será revisada antes de publicarse.',
        timer: 2500, 
        showConfirmButton: false, 
        background: '#F3E7E4', 
        color: '#16181D' 
      ...
      <div className="card shadow-sm border-0 w-100" style={{ borderRadius: '15px', backgroundColor: '#FFFFFF' }}>
        <div className="card-body p-4 p-md-5">
          ...
          <style>{`
            ...
            .pending-notice { background: #fff3cd; border: 1px solid #ffecb5; color: #664d03; border-radius: 10px; padding: 10px 16px; font-size: 0.82rem; margin-bottom: 20px; text-align: center; }
          `}</style>
            ...
            <div className={`modal-t ${isModalOpen ? 'open' : ''}`}>
              <div className="modal-box">
                ...
                <div className="pending-notice">
                  ⏳ Tu reseña será revisada por el administrador antes de publicarse.
                </div>
