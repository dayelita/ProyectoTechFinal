...
export default function TestimoniosAdmin() {
  ...
  const cargarTestimonios = async () => {
    setLoading(true);
    try {
      // 🔥 Cambio: Llamamos a la ruta especial del admin que trae TODO
      const response = await fetch(`${API_URL}/api/testimonios/admin/todos`);
      if (response.ok) {
        const data = await response.json();
        setTestimonios(data); // 🔥 Cambio: Ya no forzamos la data demo si está vacío
      }
    } catch (e) {
  ...
  const handleEliminar = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`${API_URL}/api/testimonios/eliminar/${id}`, { method: 'DELETE' });
          if (response.ok) {
  ...
  // 🔥 Cambio: Llamar al endpoint /moderar de Java enviando el JSON que espera
  ...
  // 🔥 Cambio: Ahora verificamos t.estado en lugar de t.aprobado
